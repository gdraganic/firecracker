...
pub(crate) fn parse_put_cpu_hotplug(body: &Body) -> Result<ParsedRequest, RequestError> {
    let config = serde_json::from_slice::<CpuHotplugConfig>(body.raw())?;
    Ok(ParsedRequest::new_sync(VmmAction::SetCpuHotplugConfig(
        config,
    )))
}

pub(crate) fn parse_patch_cpu_hotplug(body: &Body) -> Result<ParsedRequest, RequestError> {
    let config = serde_json::from_slice::<CpuHotplugUpdate>(body.raw())?;
